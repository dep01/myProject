...
<?php foreach($teamlist as $list): ?>
    <div class="flex-row card padding-card bg-white">
        ...
        <div class="card-body border-top border-left border-warning">
            <h5 class="card-title font-weight-bold text-dark"><?php echo $list['username']; ?></h5>
            <h5 class="card-title text-dark"><?php echo $list['fullname']; ?></h5>
            <a href="<?php echo base_url('index.php/AddTeam/'.$list['id_user']); ?>"  class="btn btn<?php echo ($list['id_list_friend_status']==1 || $list['id_list_friend_status']==3 )?'-white text-dark font-weight-bold disabled':'-dark text-white'; ?>"><?php echo ($list['id_list_friend_status'])?$list['list_friend_status']:'Follow'; ?></a> 
        </div>
